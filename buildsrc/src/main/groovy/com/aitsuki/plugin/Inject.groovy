...
public class Inject {
    ...
    public static void injectDir(String path) {
        PreDexTransform.project.logger.error "injectDir:=====$path"
        ...
        if(dir.isDirectory()) {
            dir.eachFileRecurse { File file ->
                ...
                PreDexTransform.project.logger.error "injectDir:==filePath===$filePath"
                if (filePath.endsWith(".class")
                    ...
                    PreDexTransform.project.logger.error "injectDir:==filePath===$filePath == index ===$index"

                    if (index != -1) {
                        int end = filePath.length() - 6 // .class = 6
                        String className = filePath.substring(index, end).replace('\\', '.').replace('/','.')

                        PreDexTransform.project.logger.error "injectDir:==className===$className"
    ...
    public static void injectJar(String path) {
        PreDexTransform.project.logger.error "injectJar:=====$path"
